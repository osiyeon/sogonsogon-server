...
const controller = {
  ...
  async removeBoard(req, res, next) {
    try {
      ...
      if (result1[0].count < 1) throw ({ success: `error`, result: {}, message: `게시글이 존재하지 않습니다.`});
    ...
    try {
      ...
      if (result.length < 1) throw ({ success: `error`, result: {}, message: `게시글이 존재하지 않습니다.`});
        ...
        try {
          ...
          await connection.commit();
          next({success: `ok`, result: {is_mine, board: result1[0] } , message: {}})
        } catch (e) {
          await connection.rollback();
          next(e);
        } finally {
  ...
  async myBoards(req, res, next) {
    try {
      ...
      next({success: `ok`, result: {...results}})
    } catch (e) {
      next(e);
    }
  },
